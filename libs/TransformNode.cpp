...
,world_matrix_inverted_()
,world_matrix_inverted_ptr_(&world_matrix_inverted_)
,local_matrix_inverted_()
,local_matrix_inverted_ptr_(&local_matrix_inverted_)
...
void TransformNode::refreshMatrix()
{
	if(isDirty(LOCAL)) {
		calcLocalMatrix();
		clearDirtyFlag(LOCAL);
		if(isLocalMatrixIdentity()) {
			local_matrix_inverted_ptr_ = &local_matrix_;
		}
		else {
			local_matrix_inverted_.makeInvertOf(local_matrix_);
			local_matrix_inverted_ptr_ = &local_matrix_inverted_;
		}
	}
	...
	if(isWorldMatrixIdentity()) {
		world_matrix_inverted_ptr_ = world_matrix_ptr_;
	}
	else {
		world_matrix_inverted_.makeInvertOf(*world_matrix_ptr_);
		world_matrix_inverted_ptr_ = &world_matrix_inverted_;
	}
}
...
const ofMatrix4x4* TransformNode::getWorldMatrixInverted() const
{
	return world_matrix_inverted_ptr_;
}

const ofMatrix4x4* TransformNode::getLocalMatrixInverted() const
{
	return local_matrix_inverted_ptr_;
}
